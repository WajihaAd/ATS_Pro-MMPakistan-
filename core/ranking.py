...
def _compute_risk_penalty(risk_flags: list[str]) -> float:
    """Sum the penalties for all risk flags carried by a candidate."""
    return sum(RISK_PENALTIES.get(flag, 0.0) for flag in risk_flags)
...
def _rank_rows(
    ...
    for row in rows:
        ...
        penalty = _compute_risk_penalty(flags)
