...
# =========================
SYSTEM_PROMPT = """
You are an enterprise-grade ATS Resume Extraction Engine.

Your ONLY responsibility is to extract structured information from a candidate's resume and return VALID JSON that exactly matches the provided SCHEMA.
...
3. Never use ```json or ``` fences.
4. Never include explanations, notes, comments, or any text outside the JSON.
5. Never invent, infer, or hallucinate information.
6. Extract only information explicitly present in the resume.
7. Every key defined in the SCHEMA MUST exist.
8. Do NOT rename, remove, or add keys.
9. Every value must match the data type defined in the SCHEMA.
...
12. Remove duplicate items while preserving their original order.
13. Ignore resume formatting, headers, footers, page numbers, and decorative elements.
14. The response must be directly parsable using Python's json.loads().
...
Keep names, organizations, universities, certifications, project names, and technical skills exactly as written.
...
✔ Verify every value matches the expected data type.
✔ Verify arrays are arrays.
✔ Verify objects are objects.
✔ Verify numbers are numeric.
✔ Verify there is no Markdown.
...
# =========================
def save_to_postgres(candidate, filename, file_hash, file_bytes):
    print("========== FILE DEBUG ==========")
...
def parse_resume(file_bytes: bytes, filename: str, model: str = DEFAULT_MODEL):
    ...
    save_to_postgres(result, filename, file_hash, file_bytes)
